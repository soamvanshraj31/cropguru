...
### 2.1 Fallback Mechanism (Robustness)
While CNN is our primary model, we have also implemented a lightweight **OpenCV + Random Forest** version as a fallback.
*   **Why?** If the Deep Learning model fails to load (e.g., due to memory limits on a small server) or if TensorFlow has compatibility issues, the system automatically switches to this backup model.
*   **How it works:** It uses standard image processing (Histogram of Oriented Gradients) to extract features and a Random Forest classifier to predict the disease. This ensures the system is **always available**.
